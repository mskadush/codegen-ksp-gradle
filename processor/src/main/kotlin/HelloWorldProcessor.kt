...
class HelloWorldProcessor(val codeGenerator: CodeGenerator) : SymbolProcessor {
    ...
    inner class HelloWorldVisitor : KSVisitorVoid() {
        ...
        override fun visitFunctionDeclaration(function: KSFunctionDeclaration, data: Unit) {
            ...
            val propertyPaths = collectPropertyPaths(targetType, "", mutableSetOf())
            val printlns = propertyPaths.joinToString("\n    ") { path ->
                """println("  $path")"""
            }
    ...
    /**
     * Recursively collects dotted property paths for [type] and its nested user-defined types.
     * Skips stdlib types (kotlin.*, java.*) to avoid infinite recursion into built-ins.
     * [visited] guards against cycles.
     */
    private fun collectPropertyPaths(
        type: KSType,
        prefix: String,
        visited: MutableSet<String>,
    ): List<String> {
        val decl = type.declaration as? KSClassDeclaration ?: return emptyList()
        val qualName = decl.qualifiedName?.asString() ?: return emptyList()
        if (!visited.add(qualName)) return emptyList()

        val paths = mutableListOf<String>()
        for (prop in decl.getDeclaredProperties()) {
            val path = if (prefix.isEmpty()) prop.simpleName.asString() else "$prefix.${prop.simpleName.asString()}"
            paths.add(path)
            val propType = prop.type.resolve()
            val propQualName = propType.declaration.qualifiedName?.asString() ?: continue
            if (!propQualName.startsWith("kotlin.") && !propQualName.startsWith("java.")) {
                paths.addAll(collectPropertyPaths(propType, path, visited))
            }
        }
        return paths
    }
